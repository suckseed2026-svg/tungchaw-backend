    ...
    return this.prisma.$transaction(
      async (tx) => {
        ...
        for (const item of sale.items) {
          const saleQuantity = item.quantity.plus(item.freeQuantity);

          await this.inventoryBatchesService.consumeFefo(tx, {
            businessId,
          ...
          await this.inventoryTransactionService.decreaseStock(tx, {
            ...
            referenceType: 'SALE',
            referenceId: sale.id,
            reason: 'Sale completed using FEFO batch allocation',
            createdById: userId,
          });
        }
