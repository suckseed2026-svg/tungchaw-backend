...
export class PurchaseReturnsService {
    ...
    try {
      return await this.prisma.$transaction(
        async (tx) => {
          ...
          const requestedIds = dto.items.map((item) => item.purchaseItemId);
          ...
          const calculatedItems: CalculatedPurchaseReturnItem[] = dto.items.map(
            (input, index) => {
              const purchaseItem = purchaseItemMap.get(input.purchaseItemId);
              ...
              const available = purchaseItem.quantity.minus(returned);
              ...
              const ratio = quantity.dividedBy(purchaseItem.quantity);
              ...
              const itemTax = this.money(purchaseItem.taxAmount.mul(ratio));

              const allocatedOtherCharges = purchaseItemsTotal.greaterThan(0)
                ? this.money(
              ...
              otherCharges = otherCharges.plus(allocatedOtherCharges);
  ...
  async findAll(businessId: string, query: PurchaseReturnQueryDto) {
