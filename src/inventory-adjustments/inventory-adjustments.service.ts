...
export class InventoryAdjustmentsService {
    ...
    try {
      return await this.prisma.$transaction(
        async (tx) => {
          ...
          const requestedProductIds = dto.items.map(
            (item) => item.productId,
          );
          ...
          const adjustmentNumber =
            await this.generateAdjustmentNumber(
              tx,
              businessId,
              adjustmentDate,
            );
          ...
          for (const item of calculatedItems) {
            const input = {
              ...
              reason: this.adjustmentReasonText(
                adjustmentNumber,
                dto.reason,
              ),
              notes:
                [item.itemReason, item.notes]
                  .filter(Boolean)
                  .join(' | ') || null,
              createdById: userId,
  ...
  async cancel(
    ...
    return this.prisma.$transaction(
      async (tx) => {
        ...
        if (
          adjustment.status !==
          InventoryAdjustmentStatus.COMPLETED
        ) {
          throw new BadRequestException(
  ...
  async findOne(
    ...
    const adjustment =
      await this.prisma.inventoryAdjustment.findFirst({
        where: {
          id: adjustmentId,
          businessId,
        },
  ...
  private async generateAdjustmentNumber(
    ...
    const previous = latest
      ? Number.parseInt(
          latest.adjustmentNumber.slice(prefix.length),
          10,
        )
      : 0;
